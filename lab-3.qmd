...
Question 1 a:
The county_inflation dataset has 44 rows and 30 columns. Each row represents a country and its annual inflation rates over time. The country column identifies the country, and the other 29 columns represent the annual inflation rates for each year starting at 1993 and going to 2021.
...
The three countries with the highest inflation rates in 2021 were Argentina (48.40938%), Turkiye (19.60%), and Brazil at (8.30166%). These inflation rates were higher then the United States which had an inflation rate of: 4.697859%.
All these inflation rate percentages were from 2021.
...
Based on the results - the country with the largest inflation change between the years of 1993 to 2021 is New Zealand at a ratio of 3.059378444.The infaltion increased between the years of 1993 and 2021 because the value is > 1. 
...
country_inflation_pivot <- country_inflation |>
  pivot_longer(
    cols = -country,
    names_to = "year",
...
country_inflation_pivot

# Getting the exact data set dimensions:
dim(country_inflation_pivot)

```

The new data set created from using pivot_longer has 1276 rows and 3 columns. Each rwo represents a country - year combination, with the columns country, year and annual_inflation. The year column was converted to a numeric variable using names_transform = as.numeric.
...
```
From the dataset, the highest inflation rate between 1993 and 2021 was Brazil in 1994, the inflation value was a large 2075.888%.
...
```
From the dataset, the lowest inflation rate between 1993 and 2021 was Ireland in 2009, the inflation value was at -4.478103%.
...
The highest inflation rate between 1993 and 2021 was in Brazil in 199 with a value of 2075.89% and the lowest inflation rate between 1993 and 2021 occured in Ireland in 2009, with an inflation rate of around -4.48% 
...
Why:
I chose these countries because I wanted to compare Canada to some of the strongest economies in the world. United states, France, Germany, and Japan all have strong econmies so I want to see how Canada stacks up with its inflation data.
...
```{r}
# Question 6(b)
inflation_of_countries <- country_inflation_pivot |>
...
ggplot(
  ...
  geom_point(size = 2) +
  labs(
    title = "Annual inflation rates for selected countries",
    subtitle = "Canada, United States, France, Germany, and Japan (1993-2021)",
    x = "Year",
...
Observations:
This is an interesting graph, these 5 countries seem to follow a relatively similar pattern but with all different values. For example they all initially have a drop in inflation % in the early 1990s, also around 2008-2009 there is a clear drop for all countries followed by a clear rise in 2009-2010. Also basically the United States has the highest inflation % consistently throughout and Japan has the lowest inflation throughout.
...
```
Question 8(a):
The us_inflation dataset has 132 rows and 4 columns. The dataset contains annual inflation rates for different CPI divisions in the United States for a given year, where each row represents a division and year combination and the columns are the country, cpi_division_id, year, and annual_inflation.
...
```
Question 8(b):
The cpi_division dataset has 12 rows and 2 columns. It contains the CPI division ID number and the corresponding descriptions for each CPI category.
...
```
Question 8(c):
The data sets were combined using a left join, this matches the cpi_division_id in the us_inflation dataset with id in the cpi_division dataset. The resulting dataset has 132 rows and 5 columns.The 5 column names are country, cpi_division_id, year, annual_inflation, and description.
...
```{r}
# Question 9(a):
# us_inflation_joined |> distinct(description)

division_of_interest <- c(
...
I chose these CPI divisions because they represent categories in which households spend a lot of there money on such as food, housing, transportation, health, and education. These categories are important to study since an increase in inflation directly affect these prices which correspond to every day living costs.
...
```{r}
# Question 9(b):
divisions_selected <- us_inflation_joined |>
  filter(description %in% division_of_interest)
...
ggplot(
  ...
  geom_point(size = 2) +
  labs(
    title = "Annual inflation rates for selected US CPI divisions",
    subtitle = "Food, housing, health, transport, and education (2011-2021)",
    x = "Year",
    y = "Annual inflation rate (%)",
    color = "CPI division",
    shape = "CPI division"
...
The inflation rates for Education, Housing, water, electricity and other fuels, and Food and non - Alcoholic beverages, Health are very consistent and remain stable throughout the years with just Food and Non-Alcoholic beverages dipping slightly but then coming back up. Transport is the most volatile will the largest swings and is not consistent with the other variables at all. 
